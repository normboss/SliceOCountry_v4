<?php
session_start();
require_once("dbcontroller.php");

$db_handle = new DBController();
if (!empty($_GET["action"])) {
    switch ($_GET["action"]) {
        case "add":
            if (!empty($_POST["quantity"])) {
                $code = $_POST["code"];
                // $productByCode = $db_handle->runQuery("SELECT * FROM tblproduct WHERE code='" . $_GET["code"] . "'");
                $productByCode = $db_handle->runQuery("SELECT * FROM tblproduct WHERE code='" . $code . "'");
                $itemArray = array($productByCode[0]["code"] => array('name' => $productByCode[0]["name"], 'code' => $productByCode[0]["code"], 'size' => $productByCode[0]["size"], 'quantity' => $_POST["quantity"], 'price' => $productByCode[0]["price"], 'image' => $productByCode[0]["image"]));

                if (!empty($_SESSION["cart_item"])) {
                    if (in_array($productByCode[0]["code"], array_keys($_SESSION["cart_item"]))) {
                        foreach ($_SESSION["cart_item"] as $k => $v) {
                            if ($productByCode[0]["code"] == $k) {
                                if (empty($_SESSION["cart_item"][$k]["quantity"])) {
                                    $_SESSION["cart_item"][$k]["quantity"] = 0;
                                }
                                $_SESSION["cart_item"][$k]["quantity"] += $_POST["quantity"];
                            }
                        }
                    } else {
                        $_SESSION["cart_item"] = array_merge($_SESSION["cart_item"], $itemArray);
                    }
                } else {
                    $_SESSION["cart_item"] = $itemArray;
                }
            }
            break;
        case "remove":
            if (!empty($_SESSION["cart_item"])) {
                foreach ($_SESSION["cart_item"] as $k => $v) {
                    if ($_GET["code"] == $k)
                        unset($_SESSION["cart_item"][$k]);
                    if (empty($_SESSION["cart_item"]))
                        unset($_SESSION["cart_item"]);
                }
            }
            break;
        case "empty":
            unset($_SESSION["cart_item"]);
            break;
    }
}
?>
<HTML>

<HEAD>
    <TITLE>Simple PHP Shopping Cart</TITLE>
    <link href="css/shopping-cart.css" type="text/css" rel="stylesheet" />
    <?php

    function pickleBlock()
    {
        echo '<div class="pickle-block">';
                echo '<div class="button-box">';
                    echo '<div class="text text-centered">8 ounce jar $7</div>';
                    echo '<button class="button" onclick="getShippingZipcode()">Order from Sarah</button>';
                echo '</div>';

                echo '<div class="add2cart-popup" id="get-shipping-zipcode">';
                    echo '<form method="post" action="shopping-cart.php?action=add" class="form-container">';
                        echo '<label for="quantity">Zipcode</label>';
                        echo '<input type="number" id="zipcode" min="1" value="1" name="zipcode">';
                        echo '<br>';
                        echo '<br><br>';
                        echo '<button type="submit" class="btn">Get Shipping cost</button>';
                        echo '<br><br>';
                        echo '<button type="button" class="btn cancel" onclick="cancel-get-shipping-cost()">Cancel</button>';
                    echo '</form>';

                    echo '<script>';
                    echo 'function getShippingZipcode() {';
                    echo 'document.getElementById("get-shipping-zipcode").style.display = "block";';
                    echo '}';

                    echo 'function cancel-get-shipping-cost() {';
                    echo 'document.getElementById("get-shipping-zipcode").style.display = "none";';
                    echo '}';
                    echo '</script>';

                echo '</div>';
            echo '</div>';
        echo '</div>';
    }
?>
</HEAD>

<BODY>
    <div id="shopping-cart">
        <div class="txt-heading">Shopping Cart</div>
        <?php
    // pickleBlock(    );
    ?>

        <a id="btnEmpty" href="shopping-cart.php?action=empty">Empty Cart</a>
            <table class="tbl-cart" cellpadding="10" cellspacing="1">
                <tbody>
                    <tr>
                        <th style="text-align:left;">Name</th>
                        <th style="text-align:left;">Size (ounces)</th>
                        <th style="text-align:right;" width="5%">Quantity</th>
                        <th style="text-align:right;" width="10%">Unit Price</th>
                        <th style="text-align:right;" width="10%">Price</th>
                        <th style="text-align:center;" width="5%">Remove</th>
                    </tr>
                    <?php
                    foreach ($_SESSION["cart_item"] as $item) {
                        $item_price = $item["quantity"] * $item["price"];
                    ?>
                        <tr>
                            <td><img src="<?php echo $item["image"]; ?>" class="cart-item-image" /><?php echo $item["name"]; ?></td>
                            <td><?php echo $item["size"]; ?></td>
                            <td style="text-align:right;"><?php echo $item["quantity"]; ?></td>
                            <td style="text-align:right;"><?php echo "$ " . $item["price"]; ?></td>
                            <td style="text-align:right;"><?php echo "$ " . number_format($item_price, 2); ?></td>
                            <td style="text-align:center;"><a href="shopping-cart.php?action=remove&code=<?php echo $item["code"]; ?>" class="btnRemoveAction"><img src="icon-delete.png" alt="Remove Item" /></a></td>
                        </tr>
                    <?php
                        $total_quantity += $item["quantity"];
                        $total_price += ($item["price"] * $item["quantity"]);
                    }
                    ?>

                    <!-- <tr>
                    <td colspan="2" align="right">Shipping:</td>
                        <td align="right">
                            
                        </td>
                        <td align="right" colspan="2"><a href="shopping-cart-shipping.php">Compute Shipping</a></td>
                        <td></td>
                    </tr> -->
                    <tr>
                        <td colspan="2" align="right">Total:</td>
                        <td align="right"><?php echo $total_quantity; ?></td>
                        <td align="right" colspan="2"><strong><?php echo "$ " . number_format($total_price, 2); ?></strong></td>
                        <td></td>
                    </tr>
                </tbody>
            </table>
        <?php
        } else {
        ?>
            <div class="no-records">Your Cart is Empty</div>
        <?php
        }
        ?>
    </div>

    <main class="main">
        <div class="continue-shopping">
            <button onclick="goBack()">Continue Shopping</button>
            <a href="checkout-info.php"><button>Proceed to Checkout</button></a>
            </div>
    </main>

    <script>
        function goBack() {
            window.history.back();
        }
    </script>

</BODY>

</HTML>